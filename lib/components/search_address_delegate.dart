...
class SearchAddressDelegate extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    final enderecos = enderecoProvider.enderecoFirebase;
    final listarEnderecos = query.isEmpty
        ? enderecos
        : enderecos
            .where(
              (p) =>
                  ...
                  p.cpfcliente!.toLowerCase().contains(query.toLowerCase()) ||
                  p.cidade!.toLowerCase().contains(query.toLowerCase()),
            )
            .toList();

    return ListView.builder(
      itemCount: listarEnderecos.length,
      itemBuilder: (context, index) {
        final endereco = listarEnderecos[index];
        return ListTile(
